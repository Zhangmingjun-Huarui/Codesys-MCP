...
import { LauncherConfig, IpcResult, ScriptExecutor, ScriptParams } from './types';
...
export class HeadlessExecutor implements ScriptExecutor {
  ...
  /** Execute a script by name using --runscript (loads template and interpolates params) */
  async executeRunScript(scriptName: string, params: ScriptParams, timeoutMs?: number): Promise<IpcResult> {
    ...
    return this.executeScript(interpolated, timeoutMs);
  }
}
