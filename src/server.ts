...
export async function startMcpServer(config: ServerConfig): Promise<void> {
  ...
  serverLog.info(`Starting CODESYS Persistent MCP Server v2.1.0`);
  ...
  // Note: scan_devices has been removed - use direct --runscript for device scanning

  s.tool(
    'connect_to_device',
    'Connects (logs in) to the PLC runtime for the active application. Uses IPC exec within persistent instance. Requires a configured device/gateway in the project.',
    {
      projectFilePath: z.string().describe("Path to the project file."),
    },
    async (args: { projectFilePath: string }) => {
      ...
      if (success) {
        ...
        const text = [
          `Connected to device for ${args.projectFilePath}.`,
          `Application: ${appMatch ? appMatch[1].trim() : 'Unknown'}`,
          `State: ${stateMatch ? stateMatch[1].trim() : 'Unknown'}`,
        ].join('\n');
        return { content: [{ type: 'text' as const, text }], isError: false };
      }
      return formatToolResponse(result, `Connect to device for ${args.projectFilePath}.`);
    }
  );
  ...
  s.tool(
    ...
    async (args: { projectFilePath: string; variablePath: string; value: string }) => {
      ...
      const success = result.success && result.output.includes('SCRIPT_SUCCESS');
      if (success) {
        const verifiedMatch = result.output.match(/Value Verified:\s*(.+)/);
        let text = `Variable '${args.variablePath}' set to '${args.value}'.`;
        if (verifiedMatch) {
          text += ` Verified: ${verifiedMatch[1].trim()}`;
        }
        return { content: [{ type: 'text' as const, text }], isError: false };
      }
      return formatToolResponse(
