...
try:
    ...
    def process_command(command_file):
        ...
        _log("Processing command file: %s" % command_file)
        ...
        def execute_on_ui():
            ...
            sys.stderr = capture
            try:
                script_file_path = os.path.join(COMMANDS_DIR, "%s_exec.py" % request_id)
                with open(script_file_path, "w") as sf:
                    sf.write(script_code)
                compiled = compile(script_code, script_file_path, 'exec')
        ...
        _log("Marshaling to primary thread for request %s..." % request_id)
        try:
            se.system.execute_on_primary_thread(execute_on_ui)
        except Exception as marshal_err:
            _log("Marshal error for request %s: %s" % (request_id, marshal_err))
            shared_result[0] = {
                "requestId": request_id,
                "success": False,
                "output": "",
        ...
        _log("Waiting for primary thread execution of %s (max 120s)..." % request_id)
