...
try:
    ...
    # Import CODESYS globals for login
    try:
        from scriptengine import OnlineChangeOption as _OnlineChangeOption
        _has_change_option = True
    except (ImportError, AttributeError):
        _has_change_option = False

    # Login with online change option if available, then download
    print("DEBUG: Logging in for download...")

    if hasattr(online_app, 'login'):
        if _has_change_option:
            try:
                online_app.login(_OnlineChangeOption.Try, True)
                print("DEBUG: Logged in with OnlineChangeOption.Try and auto-start=True.")
            except Exception as e:
                print("DEBUG: Login with OnlineChangeOption.Try failed: %s, trying plain login..." % e)
                online_app.login()
        else:
